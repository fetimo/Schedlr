...
Schedlr = new Ext.Application({
	...
	launch: function() {
			...
			dockedItems: [Schedlr.detailToolbar, 
				{
					...
					items: [{
						...
						iconMask: true,
						handler: function(){
							Schedlr.Viewport.setActiveItem('calendar', {type:'fade', duration:500});
						}
					},
					{
						text: 'Edit',
						...
						iconMask: true,
						handler: function() {
							Schedlr.Viewport.setActiveItem('listwrapper', {type:'fade', duration:500});
						}
					}]
				}
			]
		...
		Schedlr.indexPanel = new Ext.Panel({
			id: 'indexpanel',
			items: [
				{xtype: 'button', ui:'action', text: 'Conferences'},
				{xtype: 'button', ui:'action', text: 'Exhibitions'},
				{xtype: 'button', ui:'action', text: 'Films'},
				{xtype: 'button', ui:'action', text: 'Mixed'},
				{xtype: 'button', ui:'action', text: 'Music'},
				{xtype: 'button', ui:'action', text: 'Workshops'}
			],
			defaults: {handler: filteredList}
		});
		...
		Schedlr.Viewport = new Ext.Panel ({
			...
			items: [Schedlr.indexPanel, Schedlr.listWrapper, Schedlr.detailPanel]
		});
	}
});